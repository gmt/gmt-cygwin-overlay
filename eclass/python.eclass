...
_python_check_python_abi_matching() {
	echo "_python_check_python_abi_matching() " >&2
...
_python_package_supporting_installation_for_multiple_python_abis() {
	echo "_python_package_supporting_installation_for_multiple_python_abis() " >&2
	if has "${EAPI:-0}" 0 1 2 3 4; then
		if [[ -n "${SUPPORT_PYTHON_ABIS}" ]]; then
			echo "_python_package_supporting_installation_for_multiple_python_abis() -> 0" >&2
			return 0
		else
			echo "_python_package_supporting_installation_for_multiple_python_abis() -> 1" >&2
			return 1
		fi
	else
...
_python_parse_PYTHON_DEPEND() {
	echo "_python_parse_PYTHON_DEPEND() " >&2
...
_python_initialize_prefix_variables() {
	echo "_python_initialize_prefix_variables() " >&2
...
_python_initial_sanity_checks() {
	echo "_python_initial_sanity_checks() " >&2
	if [[ "$(declare -p PYTHON_SANITY_CHECKS_EXECUTED 2> /dev/null)" != "declare -- PYTHON_SANITY_CHECKS_EXECUTED="* || " ${FUNCNAME[@]:1} " =~ " "(python_set_active_version|python_pkg_setup)" " && -z "${PYTHON_SKIP_SANITY_CHECKS}" ]]; then
		echo "_python_initial_sanity_checks() (1): running readlink ${EPREFIX}/usr/bin/python -> $(readlink ${EPREFIX}/usr/bin/python)" >&2
		...
		echo "_python_initial_sanity_checks() (2)" >&2
		...
		echo "_python_initial_sanity_checks() (3)" >&2
	fi
}

_python_final_sanity_checks() {
	echo "_python_final_sanity_checks() " >&2
...
_python_set_color_variables() {
	echo "_python_set_color_variables() " >&2
...
_python_check_python_pkg_setup_execution() {
	echo "_python_check_python_pkg_setup_execution($@)" >&2
...
python_convert_shebangs() {
	echo "python_convert_shebangs() " >&2
	_python_check_python_pkg_setup_execution
...
python_clean_installation_image() {
	echo "python_clean_installation_image() " >&2
...
_python_calculate_PYTHON_ABIS() {
	echo "_python_calculate_PYTHON_ABIS() " >&2
	if ! _python_package_supporting_installation_for_multiple_python_abis; then
		die "${FUNCNAME}() cannot be used in ebuilds of packages not supporting installation for multiple Python ABIs"
	fi
...
python_execute_function() {
	echo "python_execute_function($@) " >&2
	if ! _python_package_supporting_installation_for_multiple_python_abis; then
		die "${FUNCNAME}() cannot be used in ebuilds of packages not supporting installation for multiple Python ABIs"
	fi
	...
	echo "python_execute_function() (1)" >&2
	while (($#)); do
		case "$1" in
			--action-message)
	...
	echo "python_execute_function() (2)" >&2

	if [[ "${default_function}" == "0" ]]; then
		echo "python_execute_function() (2a)" >&2
		if [[ "$#" -eq 0 ]]; then
		...
		echo "python_execute_function() (2b)" >&2
		if has "${EAPI:-0}" 0 1; then
	...
	echo "python_execute_function() (3)" >&2
	...
	echo "python_execute_function() (4)" >&2

	_python_calculate_PYTHON_ABIS

	echo "python_execute_function() (5)" >&2

	if [[ "${final_ABI}" == "1" ]]; then
		echo "python_execute_function() (5a)" >&2
		iterated_PYTHON_ABIS="$(PYTHON -f --ABI)"
	else
		echo "python_execute_function() (5b)" >&2
		iterated_PYTHON_ABIS="${PYTHON_ABIS}"
	fi
	echo "python_execute_function() (6)" >&2
	for PYTHON_ABI in ${iterated_PYTHON_ABIS}; do
		echo "python_execute_function() (6-for-abi (${PYTHON_ABI})" >&2
		...
		echo "python_execute_function() (6-for-abi (${PYTHON_ABI}) (1)" >&2
		_python_prepare_flags

		echo "python_execute_function() (6-for-abi (${PYTHON_ABI}) (2)" >&2
		...
		echo "python_execute_function() (6-for-abi (${PYTHON_ABI}) (3)" >&2
		...
		echo "python_execute_function() (6-for-abi (${PYTHON_ABI}) (4)" >&2
		...
		echo "python_execute_function() (6-for-abi (${PYTHON_ABI}) (5)" >&2
		...
		echo "python_execute_function() (6-for-abi (${PYTHON_ABI}) (6)" >&2
		...
		echo "python_execute_function() (6-for-abi (${PYTHON_ABI}) (7)" >&2
	...
	echo "python_execute_function() (7)" >&2
...
python_generate_wrapper_scripts() {
	echo "python_generate_wrapper_scripts() " >&2
	if [[ "${EBUILD_PHASE}" != "install" ]]; then
		die "${FUNCNAME}() can be used only in src_install() phase"
	fi
...
python_merge_intermediate_installation_images() {
	echo "python_merge_intermediate_installation_images() " >&2
	if [[ "${EBUILD_PHASE}" != "install" ]]; then
		die "${FUNCNAME}() can be used only in src_install() phase"
	fi
...
python_set_active_version() {
	echo "python_set_active_version() " >&2
	if [[ "${EBUILD_PHASE}" != "setup" ]]; then
		die "${FUNCNAME}() can be used only in pkg_setup() phase"
	fi
...
_python_get_implementation() {
	echo "_python_get_implementation() " >&2
...
python_get_implementation() {
	echo "python_get_implementation() " >&2
	_python_check_python_pkg_setup_execution
...
python_get_implementational_package() {
	echo "python_get_implementational_package() " >&2
	_python_check_python_pkg_setup_execution
...
python_get_implementation_and_version() {
	echo "python_get_implementation_and_version() " >&2
	_python_check_python_pkg_setup_execution
...
python_execute_nosetests() {
	echo "python_execute_nosetests() " >&2
	_python_check_python_pkg_setup_execution
	_python_set_color_variables
...
_python_clean_compiled_modules() {
	echo "_python_clean_compiled_modules() " >&2
	_python_initialize_prefix_variables
	_python_set_color_variables
