...
src_unpack() {
	...
	[[ ${CHOST} == *-cygwin* ]] && { use static-libs || { \
		ewarn
		ewarn "the 'static-libs' useflag is recommended on cygwin"
		ewarn "but it's not enabled now, so please go fix that"
		ewarn "unless you have a good reason for disabling it, i.e.:"
		ewarn
		ewarn "# echo 'sys-libs/ncurses static-libs' >> ${EPREFIX%/}/etc/portage/package.use"
		ewarn
	} ; }
...
#	epatch "${FILESDIR}"/${PN}-5.6-libtool.patch # used on aix
	...
	# somehow this got lost in the shuffle -- put it back
	epatch "${FILESDIR}"/${PN}-5.7-widec-wchar_h.patch

	# patch of mysterious mystery... where does it come from?
	[[ ${CHOST} == *-cygwin* ]] && \
		epatch "${FILESDIR}"/${PN}-5.7-cygwin-misc.patch

	# irix /bin/sh is no good -- FIXME: prefixify?
	[[ ${CHOST} == *-irix* ]] && \ 
...
src_compile() {
	...
	tc-export BUILD_CC
	tc-is-cross-compiler && export BUILD_CPPFLAGS+=" -D_GNU_SOURCE" #214642
...
do_compile() {
	...
	if [[ ${CHOST} == *-cygwin* ]] ; then
		# override some of the conf_abi defaults
...
src_install() {
	...
	# no need on cygwin and can cause problems with rebase
	[[ ${HOST} != *-cygwin* ]] && \
